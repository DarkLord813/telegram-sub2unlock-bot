...
class FileSharingBot:
    ...
    async def send_message(self, chat_id: int, text: str, kb=None, retries: int = 3):
    ...
    # SHOW MENUS
    # ============================================
    async def show_main_menu(self, chat_id: int, user_id: int, first_name: str):
        """Show main menu"""
        ...
        await self.send_message(chat_id, msg, InlineKeyboardMarkup(kb))
    ...
    async def show_manage_channels(self, chat_id: int, user_id: int):
        ...
        if channels:
            ...
            await self.send_message(chat_id, text, InlineKeyboardMarkup(kb))
        else:
            ...
            kb = [
                [InlineKeyboardButton('➕ Add Public Channel', callback_data='addchannel')],
                [InlineKeyboardButton('🔒 Add Private Channel', callback_data='addprivate')],
    ...
    async def show_expiry_options(self, chat_id: int):
        ...
        await self.send_message(chat_id, '⏰ Set expiry time:', InlineKeyboardMarkup(kb))
    ...
    # HANDLE FILE UPLOAD
    # ============================================
    async def handle_file_upload(self, update: Update, file_type: str):
    ...
    def setup_handlers(self):
        ...
        app.add_handler(MessageHandler(filters.VIDEO, self.video_handler))
    ...
    async def start_command(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if link:
            ...
            if not file:
                await self.send_message(chat_id, '❌ Invalid or expired link.')
                return

            if file.get('expiry') and datetime.fromisoformat(file['expiry']) < datetime.now():
                await self.db.delete_file(file['id'])
                await self.send_message(chat_id, '❌ This file has expired.')
                return
            ...
            try:
                ...
                await self.send_message(chat_id, '❌ Failed to send file.')
            return

        await self.show_main_menu(chat_id, user_id, name)
    ...
    async def document_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        await self.handle_file_upload(update, 'document')

    async def photo_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle photo uploads"""
        await self.handle_file_upload(update, 'photo')

    async def video_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        """Handle video uploads"""
        await self.handle_file_upload(update, 'video')
    ...
    async def text_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if msg.forward_from_chat:
            ...
            if session and session.get('step') == 'waiting_private_channel':
                await self.handle_private_channel_detection(update)
                return
            return
        ...
        if session and session.get('step') == 'waiting_public_channel':
            channel_input = msg.text
            if channel_input:
                await self.handle_public_channel_add(user_id, chat_id, channel_input)

    # ============================================
    # CHANNEL DETECTION HANDLERS
    # ============================================
    async def handle_private_channel_detection(self, update: Update):
    ...
    async def handle_public_channel_add(self, user_id: int, chat_id: int, channel_input: str):
        ...
        if exists:
            await self.send_message(chat_id, '⚠️ This channel is already in your list.')
            self.sessions.pop(user_id, None)
    ...
    async def callback_handler(self, update: Update, context: ContextTypes.DEFAULT_TYPE):
        ...
        if data == 'check_required_join':
            ...
            if all_joined:
                await self.db.mark_required_joined(user_id)
                try:
                    await self.application.bot.delete_message(chat_id, msg_id)
                except Exception:
                    pass
                await self.send_message(chat_id, '✅ Thank you for joining all required channels!')
                user = await self.db.get_user(user_id)
                await self.show_main_menu(chat_id, user_id, user['first_name'] if user else 'User')
            else:
        ...
        if data.startswith('joined_channels_'):
            ...
            try:
                if file.get('file_id'):
                    ...
                    await self.send_message(chat_id, '❌ Failed to send file. The file data is incomplete.')
            except Exception as e:
                logger.error(f'❌ Send file error: {e}')
                await self.send_message(chat_id, '❌ Failed to send file. Please try again.')
            return
...
async def main():
    ...
    try:
        ...
        logger.info('🔐 Bot-wide Required Channels:')
        logger.info('  📢 @NCK_Dev (Public)')
        logger.info('  🔒 Private Channel (ID: -1004266231051)')
